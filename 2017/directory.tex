...
\setupinteraction[state=start, title={2017 O'Hara Elementary Student Directory}]
...
\startmode[proof]
\title{O'Hara Elementary Directory \color[red]{PROOF DOCUMENT} 2017-09-30}
\stopmode
...
function unlisted(r)
  if r['Returned?'] == "" then return true end
...
function printnames(names, proof)
  for i,n in ipairs(names) do
    if not proof then
      ...
      if n['suffix'] ~= '' then
        s = s .. string.formatter(', %!tex!', n['suffix'])
      end
...
function printdebugimage(img, last, first, suffix, notes)
  s = string.formatter('%!tex! %!tex!, %!tex!', img, last, first)
  if suffix ~= '' then
    s = s .. string.formatter(', %!tex!', suffix)
  end
  context.bookmark({'debugno'}, s)
...
function printdirectory(proof)
  ...
  local f = readdata(io.loaddata('responses-20170926.list'))
  local prev = nil
  local cur = nil
  local names = {}
  local i = nil
    ...
    if cur == nil or not issamehomes(prev, cur) then
      ...
      if proof then
        for k,v in ipairs(names) do
          printdebugimage(string.format('%s', v['no']), v['last'], v['first'], v['suffix'], v['notes'])
          printentry(names, prev, proof)
          context.pagebreak()
          if v['extrano'] ~= "" then
            printdebugimage(string.format('%s', v['extrano']), v['last'], v['suffix'], v['first'], v['notes'])
            printentry(names, prev, proof)
            context.pagebreak()
          end
        end
...
function printstudents()
  local f = readdata(io.loaddata('students-20170926.list'))
  local grade = nil
  local teacher = nil
  local inlines = false
...
local samplenames = {{first='First', last='Last', suffix='Jr.', teacher='TEACHER', grade='1'},
                     {first='Second', last='Last', suffix='', teacher='PROFESSOR', grade='5'}}
...
if (tex.modes["proof"]) then
  ...
  justemailentry['EMAIL 1'] = 'just@example.com'
  printentry({{first='Just', last='Email', suffix='', teacher='TEACHER', grade='K'}},
             justemailentry)

  local justemailentry2 = emptyentry()
  justemailentry2['EMAIL 2'] = 'just2@example.com'
  printentry({{first='Another', last='Email', suffix='', teacher='TEACHER', grade='K'}},
             justemailentry2)
